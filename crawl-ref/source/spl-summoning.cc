...
bool summon_berserker(int pow, actor *caster, monster_type override_mons)
{
    ...
    else
    {
        ...
        {
            // trolls
            mon = random_choose_weighted(4, MONS_TROLL,
                                         3, MONS_DEEP_TROLL);
        }
        else
        {
...
spret_type cast_summon_lightning_spire(int pow, god_type god, bool fail)
{
    mgen_data spire = _pal_data(MONS_LIGHTNING_SPIRE, 2, god, SPELL_SUMMON_LIGHTNING_SPIRE);
	spire.flags |= MG_FORCE_BEH | MG_AUTOFOE;
    spire.set_summoned(&you, 2, SPELL_SUMMON_LIGHTNING_SPIRE,  god);
    spire.hd = max(1, pow);
